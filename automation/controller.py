...
def validator(policy, exec_result):
    if not exec_result.get('ok'):
        return {'pass': False, 'code': exec_result.get('code') or FAIL_CODES['EXECUTOR_NONZERO']}
    ...
    if must_next and must_next not in out:
        return {'pass': False, 'code': FAIL_CODES['NO_NEXT_STEP']}
    if must_evidence and must_evidence not in out:
        return {'pass': False, 'code': FAIL_CODES['NO_EVIDENCE']}
    if 'ROUTE_CHANGED' in out:
        return {'pass': False, 'code': FAIL_CODES['ROUTE_DEVIATION']}
...
def main():
    ...
    while True:
        ...
        evt = {'ts': now(), 'status': status, 'plan': plan, 'validator': val, 'executor': {'rc': ex.get('rc'), 'stdout': ex.get('stdout', '')[:500], 'stderr': ex.get('stderr', '')[:300]}}
        log(evt)
